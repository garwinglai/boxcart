...
export async function findUserReferralCode(code) {
	try {
		...
		console.log("file: helper/server/db. findUserReferralCode error:", error);
		...
		if (error instanceof Prisma.PrismaClientKnownRequestError) {
			...
			console.log(
				"file: helper/server/db. findUserReferralCode prismError error:",
				prismaError
			);
...
export async function updateReferralUser(codeUsed) {
	try {
		...
		console.log("file: helper/server/db. updateReferralUser error:", error);
		...
		if (error instanceof Prisma.PrismaClientKnownRequestError) {
			...
			console.log(
				"file: helper/server/db. updateReferralUser prismaError error:",
				prismaError
			);
