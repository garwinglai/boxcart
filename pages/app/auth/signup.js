	...
	async function handleNextStep() {
		...
		if (step == 0) {
			if (accessCode !== "") {
				...
				const responseOne = await checkAccessCode(accessCode);
				// value == user
				if (responseOne.success) {
					if (!responseOne.value) {
						setOpenError(true);
						setErrorMessage(
					...
					if (responseOne.value) {
						const responseTwo = await checkAccessCodeUsed(accessCode);

						if (responseTwo.success) {
							if (responseTwo.value) {
								setOpenError(true);
								setErrorMessage(
									"Account has already been created with this account. Contact hello@boxcart.shop with any questions."
								);
								setIsLoading(false);
								return;
							}
							if (!responseTwo.value) {
								const { fName, lName, email, subdomain } = responseOne.value;
